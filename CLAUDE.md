...
1. **開発サーバーを起動しておく**（ユーザー側で実施）
2. **動作確認** - ローカルで実際に動作することを確認

### 【重要】Claudeでの作業時のテスト実行

**Claudeで作業する際は、テスト実行に以下を使用してください：**
...
- **並列テスト（推奨）**: `npm run test:parallel` - 全テストを並列実行（約80秒）
- **スモークテスト**: `npm run test:smoke` - 基本動作の確認（約10秒）
- **個別テスト**: `node tests/e2e/test-[名前].cjs` - 特定の機能のテスト
- **安定版テスト**: `node tests/e2e/run-tests-stable.cjs` - グループ化実行（約2-3分）
- **フルテスト**: `npm test` - 全E2Eテスト実行（約4分、デバッグ用）

#### PRでのテスト実行（/testコマンド）

PRのコメント欄で以下のコマンドを入力するとテストが実行されます：

- `/test` - フルテストを並列実行（11個のテスト、約1.5分）
- `/test smoke` - スモークテストを実行（基本動作のみ、約25秒）
- `/test full` - フルテストを並列実行（明示的に指定）

実行すると：
1. コメントに 🚀 リアクションが付く
2. テスト開始の通知コメントが投稿される
3. ActionsタブでPRのブランチでテストが実行される

#### GitHub Actionsでの手動実行

- **手動実行のみ**: 自動実行は無効化されています
- **実行方法**: GitHubのActionsタブ → E2E Tests → Run workflow
- **オプション**: テストタイプ（full/smoke）とブランチを選択可能

#### テスト実行のコツ

- **ローカルでの確認**: 開発中は並列テストで素早く確認（約80秒）
...
- **テスト戦略の選択**:
  - 通常開発: `npm run test:parallel`（高速）
  - リソース制限環境: `run-tests-stable.cjs`（安定）
  - デバッグ: `npm test`（詳細ログ）
