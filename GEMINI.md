...
*   **Markdown形式の維持:** Pull Requestへのコメントは、Markdown形式で記述し、改行や書式が正しく反映されるようにします。`gh pr comment` コマンドを使用する際は、`--body-file` オプションを使用して、コメント内容を記述したファイル（例: `comment.md`）を指定することを推奨します。
