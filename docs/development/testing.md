...
| **test-powerup-system.cjs** | パワーアップシステム | シールドストーン収集、ダメージ吸収、二重ダメージ防止、PowerUpManager統合 | ~40秒 |
...
### 推奨ワークフロー
1. **開発中**: `npm run test:smoke` で素早く確認（約10秒）
2. **プッシュ前**: `git push` でLintとスモークテストが自動実行
3. **PR作成後**: コメントで `/test` を実行してフルテストを確認
...
| コマンド | 説明 | 実行時間 |
|---------|------|----------|
| `/test` | フルテストを実行（11個） | 約4分 |
| `/test smoke` | スモークテストを実行 | 約10秒 |
| `/test full` | フルテストを実行（明示的） | 約4分 |
