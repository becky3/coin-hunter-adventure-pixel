...
### 状態遷移
1. **メニュー画面** → ステージ選択
2. **インターミッション画面** → ステージ情報と残りライフを表示（2秒間）
3. **プレイ画面** → ゲームプレイ
4. **死亡時** → インターミッション画面（リスポーン）→ プレイ画面
5. **ゲームオーバー** → メニュー画面へ戻る
...
- **使用される場面**:
  - ゲーム開始時
  - プレイヤー死亡後のリスポーン時
  - ステージクリア後の次ステージ開始時（enableProgressionが有効な場合）
...
### 基本仕様
- **サイズ**: 
  - 通常時とダメージ時（小さい状態）の2種類
  - 細い通路を通れる衝突判定
- **移動速度**: 
  - 通常移動
  - ダッシュ移動（SHIFT押下で高速移動）
  - スムーズな加速カーブ（Ease Out Cubic）
- **初期ライフ**: 3
...
| 落ちる床（FallingFloor） | プレイヤーが乗ると1秒後に落下する床 |
...
### 落ちる床（FallingFloor）仕様
- **外観**: 石造りの16×16ピクセルのプラットフォーム
- **動作**: 
  - 通常時（stable）: 固定プラットフォームとして機能
  - 振動時（shaking）: プレイヤーが乗ると1秒間振動し、ひび割れアニメーション表示
  - 落下時（falling）: 重力に従って落下（復活しない）
- **判定**: 
  - プラットフォームレイヤーで衝突判定
  - プレイヤーのみ反応（敵は反応しない）
- **用途**: Stage 2「ざわめく洞窟」で使用予定
