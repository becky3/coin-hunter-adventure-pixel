...
2. 自動テストを実行
   ```bash
   npm test  # 軽量版テスト（WSL対応）
   # または
   npm run test:full  # Puppeteer完全テスト（要Chrome）
   ```
3. 基本動作確認を実施
   - テストが成功することを確認
   - 開発サーバーで実際の動作を確認（必要に応じて）
   - 新機能が正しく動作することを確認
   - 既存機能に影響がないことを確認
4. TodoListを更新
5. コミット前に必ず動作確認を行う
...
### 動作確認の手順
1. **開発サーバーの起動**（ユーザー側で実行）
   ```bash
   npm run dev
   ```
2. **ブラウザでアクセス**
   - http://localhost:3000/
   - Chrome DevToolsのConsoleタブでエラーを確認
3. **機能別の確認**
   - 新機能: 実装した機能が正しく動作するか
   - 既存機能: 他の機能に影響していないか
   - パフォーマンス: 60FPSで安定動作しているか
